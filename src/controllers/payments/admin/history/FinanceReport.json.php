...
$getPayments = $db->prepare("SELECT * FROM (SELECT 'StripeDD' AS Provider, 'Payments' AS Type, paymentsPending.Amount, paymentsPending.Type AS DebitCredit, CONCAT(users.Forename, ' ', users.Surname) AS `User`, paymentsPending.Name, MetadataJSON AS Info, payments.Status AS `Status`, payments.stripeFailureCode AS `StripeFailureCode`, paymentsPending.Date AS `Date`, payments.StripeFee AS `Fees`, payments.stripePaymentIntent AS `ID`, payments.StripeFee AS TransFee FROM ((`paymentsPending` INNER JOIN `payments` ON paymentsPending.Payment = payments.PaymentID) INNER JOIN `users` ON users.UserID = payments.UserID) WHERE payments.stripePaymentIntent IS NOT NULL AND payments.Date LIKE :searchDate AND users.Tenant = :tenant UNION ALL SELECT 'GoCardless' AS Provider, 'Payouts' AS Type, Amount, NULL AS DebitCredit, NULL AS `User`, NULL AS `Name`, NULL AS Info, NULL AS `Status`, NULL AS `StripeFailureCode`, ArrivalDate AS `Date`, Fees, NULL AS `ID`, NULL AS TransFee FROM paymentsPayouts WHERE paymentsPayouts.ArrivalDate LIKE :searchDate AND paymentsPayouts.Tenant = :tenant UNION ALL SELECT 'GoCardless' AS Provider, 'Payments' AS Type, paymentsPending.Amount, paymentsPending.Type AS DebitCredit, CONCAT(users.Forename, ' ', users.Surname) AS `User`, paymentsPending.Name, MetadataJSON AS Info, payments.Status AS `Status`, NULL AS `StripeFailureCode`, paymentsPending.Date AS `Date`, NULL AS `Fees`, NULL AS `ID`, NULL AS TransFee FROM (((`paymentsPending` INNER JOIN `payments` ON paymentsPending.Payment = payments.PaymentID) INNER JOIN `users` ON users.UserID = payments.UserID) LEFT JOIN paymentsPayouts ON paymentsPayouts.ID = payments.Payout) WHERE payments.Date LIKE :searchDate AND users.Tenant = :tenant UNION ALL SELECT 'Stripe' AS Provider, 'Payments' AS Type, stripePaymentItems.Amount, 'Payment' AS DebitCredit, CONCAT(users.Forename, ' ', users.Surname) AS `User`, CONCAT(stripePaymentItems.Name, ' ', stripePaymentItems.Description) AS `Name`, NULL AS Info, stripePayments.Paid AS `Status`, NULL AS `StripeFailureCode`, stripePayments.DateTime AS `Date`, stripePaymentItems.AmountRefunded AS `Fees`, stripePayments.Intent AS `ID`, stripePayments.Fees AS TransFee FROM ((`stripePaymentItems` INNER JOIN `stripePayments` ON stripePaymentItems.Payment = stripePayments.ID) INNER JOIN `users` ON users.UserID = stripePayments.User) WHERE stripePayments.DateTime LIKE :searchDate AND users.Tenant = :tenant AND stripePayments.Paid UNION ALL SELECT 'Stripe' AS Provider, 'Payouts' AS Type, Amount, NULL AS DebitCredit, NULL AS `User`, ID AS `Name`, NULL AS Info, NULL AS `Status`, NULL AS `StripeFailureCode`, ArrivalDate AS `Date`, NULL AS `Fees`, NULL AS `ID`, NULL AS TransFee FROM stripePayouts WHERE ArrivalDate LIKE :searchDate AND stripePayouts.Tenant = :tenant) AS UnitedTable ORDER BY UnitedTable.Date ASC, UnitedTable.User ASC");
$getPayments->execute([
