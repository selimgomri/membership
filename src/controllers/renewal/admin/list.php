					...
					?><tr class="table-danger"><?php
							...
							<td>
								...
								<?php } else if (bool($renewalItem['StripePaid'])) { ?>
									<img src="<?= autoUrl("public/img/stripe/" . $renewalItem['Brand'] . ".svg") ?>" style="height: 1rem;"> Paid by <span class="sr-only"><?= htmlspecialchars(getCardBrand($renewalItem['Brand'])) ?></span> <?= htmlspecialchars($renewalItem['Funding']) ?> card &middot;&middot;&middot;&middot; <?= htmlspecialchars($renewalItem['Last4']) ?> - <a class="font-weight-bold text-success" href="<?= htmlspecialchars(autoUrl("payments/card-transactions/" . $renewalItem['StripeDBID'])) ?>">SPM<?= htmlspecialchars($renewalItem['StripeDBID']) ?></a>
