...
function getBankLogo($bank) {
  ...
  if ($bank == "TSB BANK PLC") {
    ...
    $logo_path = autoUrl("img/directdebit/bank-logos/virginmoney");
  } else if ($bank == "MONZO BANK LIMITED") {
    $logo_path = autoUrl("img/directdebit/bank-logos/monzo");
  } else if ($bank == "AIB GROUP (UK) PLC (TRADING NAME FIRST TRUST BANK)") {
    $logo_path = autoUrl("img/directdebit/bank-logos/aib");
  } else if ($bank == "BANK OF SCOTLAND PLC") {
    $logo_path = autoUrl("img/directdebit/bank-logos/bankofscotland");
  } else if ($bank == "RBS ONE ACCOUNT") {
    $logo_path = autoUrl("img/directdebit/bank-logos/oneaccount");
  } else if ($bank == "HANDELSBANKEN PLC") {
    $logo_path = autoUrl("img/directdebit/bank-logos/handlesbanken");
  } else if ($bank == "BANK OF AMERICA, NA") {
    $logo_path = autoUrl("img/directdebit/bank-logos/bankofamerica");
  } else if ($bank == "MODULR FS LTD") {
    $logo_path = autoUrl("img/directdebit/bank-logos/modulr");
  } else if ($bank == "TRANSFERWISE LIMITED") {
    $logo_path = autoUrl("img/directdebit/bank-logos/transferwise");
  }
